...
class ServerSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        """Meta Data"""
        model = OnlineServer
        fields = ('id', 'url', 'server_type', 'server_uid', 'server_name',
                  'start_date', 'update_date', 'current_download',
                  'finish_count',
                  'error_count')
    ...
    def create(self, validated_data):
        ...
        query = OnlineServer.objects.filter(
            server_uid=server_uid)  # pylint: disable=E1101
        if query:
